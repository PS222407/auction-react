...
function WonAuctions() {
    ...
    async function getWonAuctions() {
        const [response, data] = await auth.fetchWithIntercept(
            `${config.API_URL}/api/v1/User/Auctions/Won`,
            {
                headers: {
                    ...
                    <div key={auction.id} className={'border border-black rounded'}>
                        <div className={'flex flex-col items-center'}>
                            <div className={'w-min p-3'}>
                                <div>
                                    <img className={'max-w-56 w-56 aspect-square object-cover sm:max-w-56 sm:w-56'} src={auction.product.imageUrl} alt={auction.product.name} />
                                </div>
                                <div className={'flex flex-col gap-y-3 items-center'}>
                                    ...
                                    <div className={'font-bold'}>
                                        {new Intl.NumberFormat('nl-NL', {
                                            style: 'currency',
                                            currency: 'EUR',
                                        }).format(auction.bids[0].priceInCents / 100)}
                                    </div>
