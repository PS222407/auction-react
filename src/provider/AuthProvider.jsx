...
export const AuthProvider = ({ children }) => {
    ...
    async function fetchWithIntercept(url, options, user = null) {
        ...
        const data = ![204, 401, 404].includes(response.status) ? await response.json() : null
    ...
    async function getAccessTokenRefreshIfNeeded(user) {
        ...
        const apiurl = (
            await (
                await fetch('/config.json').catch((error) => {
                    if (error.message === 'Failed to fetch') toast('Network error', { type: 'error' })
                })
            ).json()
        ).API_URL
    ...
    return <AuthContext.Provider value={{ login, logout, user, fetchWithIntercept }}>{children}</AuthContext.Provider>
}
