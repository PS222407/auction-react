...
function OrderConfirmed() {
    ...
    async function getOrder() {
        ...
        const [response, data] = await auth.fetchWithIntercept(
            `${config.API_URL}/api/v1/Order/${id}`,
            {
                headers: {
                            ...
                            <div className={'border border-black rounded'}>
                                <div className={'flex flex-col items-center'}>
                                    <div className={'w-min p-3'}>
                                        <div>
                                            <img className={'max-w-56 w-56 aspect-square object-cover sm:max-w-56 sm:w-56'} src={order.product.imageUrl} alt={order.product.name} />
                                        </div>
